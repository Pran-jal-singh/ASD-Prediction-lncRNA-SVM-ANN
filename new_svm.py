...
def train_and_evaluate():
    # Load the data
    data = pd.read_csv('augmented_data_multiple_rounds.csv')  # Ensure the file path is correct

    # Prepare the data
    X = data.drop(['gene_id', 'Gene_type'], axis=1)  # Features
    y = data['Gene_type']  # Target variable
    ...
    svm_model = SVC(kernel='rbf', probability=True, gamma='auto')
    ...
    joblib.dump(svm_model, 'svm_model_full.pkl')
    print("Full model saved as 'svm_model_full.pkl'.")
